...
let score = 0;
let timer;
const initialTotalTime = 300; // Set total time for the quiz in seconds (e.g., 5 minutes = 300 seconds)
let totalTime = initialTotalTime; // This will be used to track remaining time

function startQuiz() {
    currentQuestionIndex = 0;
    score = 0;
    totalTime = initialTotalTime; // Reset the timer to the initial value
...
function showScore() {
    clearInterval(timer); // Stop the timer when showing the final score
    resetState();
...
function startTimer() {
    timer = setInterval(() => {
        ...
        if (totalTime <= 0) {
            clearInterval(timer);
            showScore(); // Show the final score when the timer runs out
        }
    }, 1000);
}
...
function formatTime(seconds) {
    ...
    return `Time left: ${minutes}:${remainingSeconds < 10 ? '0' : ''}${remainingSeconds} min`; // Format time as minutes:seconds
}
...
nextButton.addEventListener('click', () => {
    if (currentQuestionIndex < questions.length) {
        ...
        startQuiz(); // Restart the quiz and reset the timer
    }
});
